...
`
	...
	fi, ok := createDirEntry(d)
	if ok != nil {
		t.Fatalf("unexpected not ok")
	}
	wantPos := Pos{Track: DirTrack, Sector: 1, At: 0x40}
	gotPos := fi.pos
	if wantPos != gotPos {
		t.Fatalf("wanted %+v ; got %+v", wantPos, gotPos)
	}
}

func TestCreateDirEntryNextSector(t *testing.T) {
	d := NewDisk("", "")
	dump := `
00016600  12 04 82 11 00 46 49 4c  45 20 31 a0 a0 a0 a0 a0  |.....FILE 1.....|
00016610  a0 a0 a0 a0 a0 00 00 00  00 00 00 00 00 00 01 00  |................|
...
`
	...
	//
	wantPos := Pos{Track: DirTrack, Sector: 1 + dirInterleave, At: 0}
	gotPos := fi.pos
	if wantPos != gotPos {
		t.Fatalf("wanted %+v ; got %+v", wantPos, gotPos)
	}
